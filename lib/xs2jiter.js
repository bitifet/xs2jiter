...
module.exports = function x2j(xml, bufferMaxLength){
    ...
    parser.on('endElement', function (name) {//{{{
        ...
        if (deep >= 2) {
            ...
            } else if (stack.length > 0 && !(currElem.target instanceof Object)) {
                // Propagate primitives (null, string) to parent — these
                // replaced the shared reference so parent still has old {}/placeholder.
                // Skip objects/arrays: collapse already happened inside the shared ref
                // and parent's wrapper must be preserved (e.g. {key: [...]}).
